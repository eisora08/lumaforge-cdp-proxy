...
fn handle_command(command: &str) -> String {
    ...
    match cmd {
        ...
        "active-theme" => match crate::theme::load_active_theme() {
            Some(theme) => {
                format!(
            ...
            match crate::theme::write_active_theme_name(theme_name) {
                Ok(()) => {
                    ...
                    if let Err(e) = crate::theme::export_theme_for_cef_hook() {
                        crate::log_to_temp(&format!(
                            "[steamcdp] Theme export failed after set: {}",
                            e
                        ));
                    }
                    // Signal reload
                    ...
                    format!(
                        r#"{{"status":"ok","message":"Theme set to '{}'","theme":"{}"}}"#,
        ...
        _ if cmd.starts_with("set-condition ") => {
            ...
            if parts.len() < 2 {
                return r#"{"status":"error","message":"Usage: set-condition <name> <value>"}"#
                    .to_string();
            }
            ...
            match config.save() {
                Ok(()) => {
                    ...
                    if let Err(e) = crate::theme::export_theme_for_cef_hook() {
                        crate::log_to_temp(&format!(
                            "[steamcdp] Theme export failed after condition: {}",
                            e
                        ));
                    }
                    let _ = write_theme_reload_signal();
        ...
        "theme-info" => match crate::theme::load_active_theme() {
            Some(theme) => {
                let conditions: Vec<String> = theme.manifest.conditions.keys().cloned().collect();
                let patches_info: Vec<String> = theme
                    .manifest
                    .patches
                    .iter()
                    .map(|p| {
                        format!(
                            "regex='{}' css={:?} js={:?}",
                            p.match_regex_string, p.target_css, p.target_js
                        )
                    })
                    .collect();
                format!(
